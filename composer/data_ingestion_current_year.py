...
def download_current_year_reports(): 
    ...
    for link in links:
        ...
        if sunday_date:
            ...
            logging.error(f"Report date could not be extracted for {report_title}.")
            continue
...
with DAG(
    ...
    download_current_year_reports_task = PythonOperator(
        task_id="download_current_year_reports_task",
        python_callable=download_current_year_reports,
    )
    ...
    gcs_to_bq_task = BigQueryInsertJobOperator(
        ...
        configuration={
            "load": {
                "sourceUris": [f"gs://{BUCKET}/data/{provide_file_name_from_loop}.csv"],
                "destinationTable": {
    ...
    start_task >> download_current_year_reports_task >> format_to_csv_task >> gcs_to_bq_task >> end_task
